<#
    ...
    PowerShell assertion style as Preset.ActivitySessions.Tests.ps1 (run, assert,
    ...
    Cases:
      ...
      p2. Exact aggregate schema: the final | project column list (8 columns in
          order) and the `by UPN, YearMonth` group-by key match the B4 contract.
      p3. {USERFILTER} appears AFTER `extend UPN = tolower(AccountUpn)` so the
          hash(UPN, ...) bucket predicate is valid at the filter point.
      ...
      p5. Bucket substitution: Month mode + -UserBucketColumn UPN over a
          saturated month -> at least one built KQL carries a `where hash(UPN, ...)`
          body predicate and NO leftover {USERFILTER} token, and the initial
          detection call remains the plain `where true` form.
#>
...
# PSCustomObjects with the 8 aggregate columns; row math is irrelevant -- these
...
# ===========================================================================
# CASE p2. Exact aggregate schema (B4 contract).
#   - The | project block lists exactly, in order:
#       UPN, YearMonth, TotalSessions, OffHoursSessions, OffHoursPct,
#       DistinctCountries, AnomalousCountryCount, AnomalousCountries
#     (spans two lines in the B4 body; both parts asserted).
#   - Group-by keys `by UPN, YearMonth`.
# ===========================================================================
Write-Host ""
Write-Host "---- Case p2: exact aggregate schema (project column list + group-by) ----" -ForegroundColor Cyan
try {
    $projLine1 = '| project UPN, YearMonth, TotalSessions, OffHoursSessions, OffHoursPct,'
    $projLine2 = 'DistinctCountries, AnomalousCountryCount, AnomalousCountries'
    $hasProj1   = $presetRaw.Contains($projLine1)
    $hasProj2   = $presetRaw.Contains($projLine2)
    $hasGroupBy = $presetRaw.Contains('by UPN, YearMonth')
    $passP2 = $hasProj1 -and $hasProj2 -and $hasGroupBy
    Add-CaseResult -Name 'p2. exact aggregate schema (final project 8 columns in order + group-by)' -Pass $passP2 `
        -Detail ("projLine1={0}; projLine2={1}; groupBy={2}" -f $hasProj1, $hasProj2, $hasGroupBy)
}
catch {
    Add-CaseResult -Name 'p2. exact aggregate schema (final project 8 columns in order + group-by)' -Pass $false -Detail "threw: $($_.Exception.Message)"
}

# ===========================================================================
# CASE p3. {USERFILTER} appears AFTER `extend UPN = tolower(AccountUpn)`.
...
#   of the UPN extend is LESS than the index of the body `| where {USERFILTER}`.
# ===========================================================================
Write-Host ""
Write-Host "---- Case p3: {USERFILTER} appears after `extend UPN = tolower(AccountUpn)` ----" -ForegroundColor Cyan
try {
    ...
    $idxUpn  = $presetRaw.IndexOf('extend UPN = tolower(AccountUpn)')
...
try {
    ...
    $p4Mock = {
        ...
        # Small (< RowCap) synthetic set of aggregate rows (8 columns).
        return @(
            [PSCustomObject]@{ UPN='alice@contoso.com'; YearMonth='2026-05'; TotalSessions=4; OffHoursSessions=1; OffHoursPct=0.25; DistinctCountries=1; AnomalousCountryCount=0; AnomalousCountries='' },
            [PSCustomObject]@{ UPN='bob@contoso.com';   YearMonth='2026-05'; TotalSessions=2; OffHoursSessions=0; OffHoursPct=0.0;  DistinctCountries=2; AnomalousCountryCount=1; AnomalousCountries='DE' }
        )
    }.GetNewClosure()
...
#   one built KQL carries a `where hash(UPN, ...)` body predicate and NO leftover
    ...
    $p5Mock = {
        ...
        if ($ctx.BucketCount -eq 0) {
            ...
            for ($i = 0; $i -lt $rowCap; $i++) {
                $rows.Add([PSCustomObject]@{ UPN=("user{0}@contoso.com" -f $i); YearMonth='2026-05'; TotalSessions=1; OffHoursSessions=0; OffHoursPct=0.0; DistinctCountries=1; AnomalousCountryCount=0; AnomalousCountries='' })
            }
            return @($rows)
        }
        # Bucket call: small (< RowCap) set so bucketing resolves.
        return @(
            [PSCustomObject]@{ UPN='alice@contoso.com'; YearMonth='2026-05'; TotalSessions=3; OffHoursSessions=1; OffHoursPct=0.3333; DistinctCountries=1; AnomalousCountryCount=0; AnomalousCountries='' }
        )
    }.GetNewClosure()
...
# ===========================================================================
Write-Host ""
Write-Host "==== PRESET OFF-HOURS-GEO RESULTS ====" -ForegroundColor White
$failed = 0
